...
企业级 RAG 问答系统项目。Day 1 至 Day 7 已完成；Day 7 Qdrant 接入支持多文档上传、
分页切分、显式向量索引及固定 Top 5 语义检索，`PLAN.md` 对应状态已更新。
`eefd397` 是 Day 7 提交前基线，当前检查点 SHA 应通过 Git 实时读取；Hybrid Search、
Rerank、RAG、LangGraph Agent 等仍属于后续目标。
...
`GET /health` 不依赖 PostgreSQL 或 Qdrant。搜索 Query 使用 BGE 查询 instruction；文档
Chunk 保持直接编码。当前检索接口不支持可调 top_k、doc_id filter 或 score threshold。
